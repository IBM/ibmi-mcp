  ...
  static async process(
    ...
    return ErrorHandler.tryCatch(
      async () => {
        ...
        if (detailedLogging) {
          logger.debug("SQL parameters processed with detailed stats", {
            ...context,
            ...result.stats,
            mode: result.mode,
          ...
          logger.debug("SQL parameters processed", {
            ...context,
            mode: result.mode,
  ...
  private static async processPositionalParameters(
    ...
    for (let i = 0; i < parameterCount; i++) {
      ...
      if (paramValue === undefined || paramValue === null) {
        missingParameters.push(paramName);
        logger.warning(`Positional parameter at index ${i} not found`, {
          ...context,
          availableParams: Object.keys(parameterValues),
          missingIndex: i,
        });
