...
export class YamlParser {
  ...
  static async parseYamlFile(
    ...
    return ErrorHandler.tryCatch(
      async () => {
        ...
        logger.debug("YAML file content loaded", {
          ...operationContext,
        ...
        if (sourceValidationErrors.length > 0) {
          logger.error("Source reference validation failed", {
            ...operationContext,
        ...
        if (toolsetValidationErrors.length > 0) {
          logger.error("Toolset reference validation failed", {
            ...operationContext,
  ...
  private static interpolateEnvironmentVariables(
    ...
    return content.replace(/\$\{([^}]+)\}/g, (match, varName) => {
      ...
      logger.debug(`Environment variable ${varName} found and substituted`, {
        ...context,
        varName,
  ...
  static interpolateClientEnvironmentVariables(
    ...
    logger.debug("Starting client environment variable interpolation", {
      ...operationContext,
