...
export async function generateSqlLogic(
  ...
  logger.debug("Processing generate SQL DDL logic", {
    ...context,
    toolInput: params,
  });
  ...
  try {
    ...
    if (result.data && Array.isArray(result.data)) {
      logger.debug("Processing GENERATE_SQL result data", {
        ...context,
      ...
      logger.debug("DDL extraction completed", {
        ...context,
        processedRows: resultStrings.length,
        totalDdlLength: generatedSql.length,
        avgRowLength: resultStrings.length > 0 ? Math.round(generatedSql.length / resultStrings.length) : 0,
      });
    }
    ...
    logger.debug("SQL DDL generated successfully", {
      ...context,
      objectName: params.object_name,
      objectLibrary: params.object_library,
      objectType: params.object_type,
    ...
    logger.error("SQL DDL generation failed", {
      ...context,
