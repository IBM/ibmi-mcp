...
export async function toolsetsResourceLogic(
  ...
  logger.debug("Processing toolsets resource logic", {
    ...context,
    uri: uri.href,
    params,
  });
  ...
  logger.debug("Toolsets resource processed successfully", {
    ...context,
    responseSummary: {
      toolsetsReturned: toolsets.length,
      totalToolsInResponse: toolsets.reduce((sum, ts) => sum + ts.toolCount, 0),
